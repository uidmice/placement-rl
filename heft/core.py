...
def wbar(ni, agents, compcost):
    ...
    return sum(compcost(ni, agent) for agent in agents[ni]) / len(agents)
...
def cbar(ni, nj, agents, commcost):
    ...
    n2 = len(agents[nj])
    npairs = n1 * n2
    return 1. * sum(commcost(ni, nj, a1, a2) for a1 in agents[ni] for a2 in agents[nj]
                                        if a1 != a2) / npairs
...
def allocate(job, orders, jobson, prec, compcost, commcost, agents):
...
def schedule(succ, agents, compcost, commcost):
    ...
    agents - dict of task: set of agents that can perform this task
